...
import { listConversations, updateConversationTitle, deleteConversationsByCharacter } from "@/lib/api";
        ...
        {character ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {error ? (
              ...
              // Single character view: show a New Chat button for that character at top-left
              <>
                ...
                {items.filter((x) => x.character === character).map((c) => (
                <Card key={c.id}>
                  <CardBody className="p-5 space-y-2">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      ...
                      <DeleteConversationButton id={c.id} onDeleted={() => setItems((arr) => arr.filter((x) => x.id !== c.id))} />
                    </div>
                  </CardBody>
                </Card>
                ))}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={async () => {
                          const ok = confirm(`Delete all ${charName} chats? This cannot be undone.`);
                          if (!ok) return;
                          await deleteConversationsByCharacter(charName).catch(() => {});
                          setItems((arr) => arr.filter((x) => x.character !== charName));
                        }}
                      >
                        Delete all
                      </Button>
